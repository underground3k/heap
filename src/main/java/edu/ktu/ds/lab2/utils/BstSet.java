...
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable {
    ...
    private BstNode<E> removeRecursive(E element, BstNode<E> node) {
        ...
        int comparator = c.compare(element, node.element);
...
//            if (node.left == null && node.right == null) { //jei neturi vaiku
//                node.element = null;
//
//            }

            if (node.left == null) { //jei turi viena vaika
                return node.right;
            }
            if (node.right == null) {
                return node.left;
            ...
            //jei turi 2 vaikus
            BstNode<E> pakaitalas = getMin(node.right);
            node.element = pakaitalas.element;
            node.right = removeRecursive(pakaitalas.element, node.right);
        }
    ...
    public LinkedList<E> setToLinkedList() {
        LinkedList<E> result = new LinkedList<E>();

        root = recursive(result,root);
//        result.addFirst(rootElement);


        return result;
    }
    private BstNode<E> recursive(LinkedList<E> list, BstNode<E> node) {

        if(node !=null){
            list.add(node.element);
            node.left = recursive(list,node.left);
            node.right = recursive(list,node.right);
        }
    ...
    public Set<E> subSet(E element1, E element2) {
        if (element1 == null || element2 == null) {
            throw new IllegalArgumentException("none of the elements can be null");
        }
        BstSet<E> set = new BstSet<>();
    ...
    private class IteratorBst implements Iterator<E> {
        ...
        public void remove() {
            if (last == null) {
                throw new IllegalStateException("next hasnt been called, or remove already called");
            }
